...
export async function generateMetadata({ params: { locale } }: Props): Promise<Metadata> {
  ...
  const description = isEs
    ? 'Supervisión técnica de obras y documentación aérea con drone profesional en República Dominicana. Reportes de avance, inspección remota objetiva y evidencia visual para constructoras y desarrolladores. Desde $220 USD.'
    : 'Drone-based technical construction supervision and aerial documentation in the Dominican Republic. Progress reports, remote objective inspection, and visual evidence for builders and developers. From $220 USD.'
  const keywords = isEs
...
export default async function DroneConstructionSupervisionPage({ params: { locale } }: Props) {
  ...
  const FAQS: { q: string; a: string }[] = isEs
    ? [
        { q: '¿Cuánto cuesta una inspección aérea de obra con drone?', a: 'Tres paquetes a precio fijo: $220 USD para una inspección puntual de hasta 1.5h sobre la obra (similar al Listado Esencial), $420 USD para cobertura completa con vuelos múltiples y reporte (Propiedad Premium · 3h), y $620 USD para proyectos grandes con jornada completa de 4h (Finca de Lujo). Para contratos mensuales/quincenales con cobertura recurrente cotizamos por proyecto.' },
    ...
    : [
        { q: 'How much does a drone construction inspection cost?', a: 'Three flat-rate packages: $220 USD for a one-off inspection of up to 1.5h over the site (similar to Essential Listing), $420 USD for full coverage with multiple flights + report (Premium Property · 3h), and $620 USD for large projects with a full 4h day (Luxury Estate). For monthly / bi-weekly retainer contracts we quote per project.' },
