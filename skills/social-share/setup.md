...
### 6a. Get permanent page token (System User route — survives password changes)

**Why not the Graph API Explorer route:** a Page token derived from a personal
user login is invalidated the moment that Facebook account changes its password
(or logs out everywhere). A token generated by a Business Manager **System User**
is not tied to any personal session and never expires unless revoked.

1. developers.facebook.com → My Apps → Create App → Business. Under **Use cases** add
   both "Manage everything on your Page" and "Manage messaging & content on Instagram",
   then open each and click **Add** on `pages_manage_posts`, `pages_read_engagement`,
   `pages_show_list`, `instagram_basic`, `instagram_content_publish`. A permission that
   isn't added to a use case will not appear in the token generator below.
2. business.facebook.com → Settings → Users → **System users** → Add. Role: Admin.
3. On the system user → **Assign assets**, all with Full control:
   Pages (the brand Page), Instagram accounts (the linked IG business account), Apps (the app from step 1).
4. **Generate token** → choose the app → expiration **Never** → tick
   `pages_manage_posts`, `pages_read_engagement`, `pages_show_list`,
   `instagram_basic`, `instagram_content_publish`, `business_management`.
5. The generated token identifies the *system user*, not the Page. Exchange it for
   the Page token (also permanent, because it derives from a permanent token):

```bash
# Lists every Page the system user was assigned, with a permanent Page access_token
curl "https://graph.facebook.com/v21.0/me/accounts?fields=id,name,instagram_business_account,access_token&access_token=SYSTEM_USER_TOKEN"
# → copy the Page's access_token field. Confirm with:
